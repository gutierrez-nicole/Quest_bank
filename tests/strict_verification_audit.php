...
// Fetch QA Teacher and QA Student A IDs
...
// Prepare generic statement for lesson insertions
...
try {
    // 4.4 IDOR Protection Test
    $stmtIdor = $pdo->prepare("SELECT COUNT(*) FROM exam_submissions WHERE id = ? AND student_id = ?");
    $stmtIdor->execute([$submissionId, $admin['id']]); // Student B trying to access Student A's record
...
foreach ($termsToAudit as $term) {
    ...
    $classification = "Documentation / Test Fixtures / UI Label";
    if (empty($lines)) {
        $classification = "Cleaned (Zero Runtime Matches)";
    }
    
    $auditSummary[$term] = [
        'match_count' => count($lines),
        'classification' => $classification,
        'sample_line' => $lines[0] ?? 'None'
    ];
...
// 7.2 CSRF Token Input Generation
...
// 7.3 SQL Injection Parameter Binding
