...
try {
    ...
    $runner->assertTrue("TEST 1: One Prelim Lesson Generation", isset($res1['success']) && count($res1['questions']) >= 1, "Generated " . count($res1['questions'] ?? []) . " questions");
    ...
    $runner->assertTrue("TEST 2: Multiple Prelim Lessons Pool", isset($res2['success']) && count($res2['questions']) >= 1, "Questions generated across 2 Prelim lessons");
    ...
    $runner->assertTrue("TEST 3: Prelim + Midterm Cross-Period Pool", isset($res3['success']), "Combined Prelim and Midterm pool processed");
    ...
    $runner->assertTrue("TEST 4: Full Multi-period (Prelim + Midterm + Finals)", isset($res4['success']), "Covered all 3 period lessons");
    ...
    $runner->assertTrue("TEST 5: General Lesson + Period Lesson Pool", isset($res5['success']), "Combined General and Finals pool processed");
    ...
    $runner->assertTrue("TEST 6: Mixed Subject Selection Server-Side Rejection", $mixedSubjectRejected, "Detected subjects: " . implode(', ', $subjList6));
    ...
    $runner->assertTrue("TEST 7: Unauthorized Lesson ID Security Block", $unauthBlocked, "Teacher {$teacher_id} cannot access unauthorized lesson {$l_unauth}");
    ...
    $runner->assertTrue("TEST 8: Failed Extraction Status Rejection", $status8 !== 'completed', "Status '{$status8}' correctly rejected from AI pool");
    ...
    $runner->assertTrue("TEST 9: Empty Lesson Text Rejection", empty($text9), "Empty text content correctly rejected");
    ...
    $runner->assertTrue("TEST 10: Per-Question Source Traceability in generated_question_sources", !empty($rel10) && $rel10['academic_period'] === 'midterm' && $rel10['source_topic'] === 'Effective Stress', "Question {$qId10} linked specifically to lesson {$l_midterm} (Midterm, Topic: Effective Stress)");
    ...
    $runner->assertTrue("TEST 11: Large-Context Chunking Execution & Boundaries", isset($res11['success']) && ($res11['metadata']['chunked'] ?? false) === true, "Successfully chunked large pool into " . ($res11['metadata']['chunk_count'] ?? 1) . " chunks without losing content");
    ...
    $runner->assertTrue("TEST 12: Generation Audit Batch Record persistence (ai_generation_batches)", !empty($batchRec12) && $batchRec12['requested_question_count'] == 5, "Audit Batch Record ID {$batchDbId12} persisted with exact metadata");
    ...
    $runner->assertTrue("TEST 13: Single-Lesson Generation Backward Compatibility", isset($res13['success']) && count($res13['questions']) >= 1, "Single-lesson flow unaffected");
    ...
    $runner->assertTrue("TEST 14: Qualifying Exam Rules Unaffected", $qualRec['exam_category'] === 'qualifying' && floatval($qualRec['qualifying_passing_percentage']) === 85.0, "Category: qualifying, Passing Pct: 85%");
