...
try {
    ...
    logTest("TEST 1: One Prelim Lesson fetch & validation", !empty($res1) && $res1['academic_period'] === 'prelim', "ID: {$l_prelim1}, Period: prelim");
    ...
    logTest("TEST 2: Multiple Prelim Lessons pool", count($res2) === 2 && array_unique($res2) === ['prelim'], "Lessons retrieved: " . count($res2));
    ...
    sort($res3);
    logTest("TEST 3: Cross-period (Prelim + Midterm)", $res3 === ['midterm', 'prelim'], "Periods covered: " . implode(', ', $res3));
    ...
    sort($res4);
    logTest("TEST 4: Full Multi-period (Prelim + Midterm + Finals)", count($res4) === 3, "Covered periods: " . implode(', ', $res4));
    ...
    sort($res5);
    logTest("TEST 5: General lesson + Period lesson", $res5 === ['finals', 'general'], "Periods: " . implode(', ', $res5));
    ...
    logTest("TEST 6: Unauthorized Lesson ID security block", $unauth_blocked, "Unauthorized ID {$l_unauth} successfully excluded");
    ...
    logTest("TEST 7: Different subject detection", $diff_subj_detected, "Subjects detected: " . implode(', ', array_unique($res7)));
    ...
    logTest("TEST 8: Incomplete extraction lesson block", $res8 === false, "Processing status 'processing' correctly blocked from generation");

    
    $stmt = $pdo->prepare("SELECT lesson_text FROM lesson_materials WHERE id = ?");
    $stmt->execute([$l_empty]);
    $res9 = trim($stmt->fetchColumn());
    logTest("TEST 9: Empty lesson text block", empty($res9), "Empty text content correctly identified");

    
    $dupes = [$l_prelim1, $l_prelim1, $l_midterm];
    $unique_dupes = array_unique(array_map('intval', $dupes));
    logTest("TEST 10: Duplicate Lesson IDs deduplication", count($unique_dupes) === 2, "Duplicate inputs de-duplicated to: " . implode(', ', $unique_dupes));

    
    $large_pool = array_fill(0, 100, $l_prelim1);
    $unique_large = array_unique($large_pool);
    logTest("TEST 11: Large lesson pool deduplication & sanity", count($unique_large) === 1, "100 duplicate items deduplicated safely");
    ...
    logTest("TEST 12: Source relation table persistence (generated_question_sources)", !empty($res12) && $res12['academic_period'] === 'midterm', "Relation ID {$rel_id} bound to question {$question_id}");
    ...
    logTest("TEST 13: Existing single-lesson generation backward compatibility", $single_exam_id > 0, "Single-lesson exam created with ID {$single_exam_id}");

    
    $stmtReg = $pdo->prepare("SELECT exam_category FROM exams WHERE id = ?");
    $stmtReg->execute([$exam_id]);
    $catReg = $stmtReg->fetchColumn();
    logTest("TEST 14: Regular exam unaffected by cross-period metadata", $catReg === 'regular' || $catReg === null, "Category: " . ($catReg ?: 'regular'));
    ...
    logTest("TEST 15: Qualifying exam rules unaffected", $res15['exam_category'] === 'qualifying' && floatval($res15['qualifying_passing_percentage']) === 80.0, "Category: qualifying, Pass: 80%");
