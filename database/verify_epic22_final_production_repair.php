...
try {
    ...
    logTest("Setup: Database Connection Established", false, $e->getMessage());
    exit(1);
}
...
logTest("TEST 1: Multi-Chunk Structure and Result Tracking", $pass1, "Chunks created: " . count($res1['metadata']['chunk_generation_results'] ?? []));
...
logTest("TEST 2: Per-Chunk Metadata Schema Completeness", $validChunkSchema, "All required 9 per-chunk tracking keys present in every chunk result");
...
logTest("TEST 3: Post-Generation Coverage Metrics Computation", $pass3, "Per-lesson and per-period question counts calculated for all 3 lessons and periods");
...
logTest("TEST 4: Uncovered Lesson and Period Detection Contract", $pass4, "uncovered_lesson_ids and uncovered_periods exported as arrays");
...
logTest("TEST 5: Exact Requested Final Question Count & Deduplication", $pass5, "Requested 9 questions, generated exactly 9 non-duplicate questions");
...
logTest("TEST 6: Batch Status Classification", $pass6, "batch_status evaluated as '{$res6['metadata']['batch_status']}'");
...
logTest("TEST 7: Database Migration & Coverage Metadata Persistence", $pass7, "chunk_generation_results, questions_per_lesson, questions_per_period persisted into ai_generation_batches");
...
if ($failCount === 0) {
    echo "RESULT: SUCCESS — All coverage-aware refill and metadata assertions passed cleanly.\n";
    exit(0);
} else {
    echo "RESULT: FAILURE — {$failCount} test(s) failed.\n";
    exit(1);
}
