...
try {
    $maintenanceMode = (SystemSettingsService::getSetting('maintenance_mode', 'off') === 'on');
} catch (Exception $e) {
    $maintenanceMode = false;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['action_register'])) {
        ...
        if ($maintenanceMode) {
            $error_msg = "Public account registration is disabled during system maintenance.";
        } else {
            ...
            if ($requestedRole === 'admin' || $requestedRole === 'teacher') {
                $error_msg = "Public registration is permitted for Student accounts only. Teacher and Admin accounts must be created by an administrator.";
    ...
    <div class="w-full md:w-7/12 lg:w-1/2 flex flex-col justify-center items-center p-6 md:p-12 relative overflow-y-auto custom-scrollbar h-2/3 md:h-full">
        ...
        <div class="w-full max-w-md mb-4 space-y-2">
            <?php if ($maintenanceMode): ?>
                <div class="bg-amber-500/10 border border-amber-500/30 p-3.5 rounded-xl text-xs text-amber-600 dark:text-amber-400 font-bold flex items-center gap-2">
                    <i class="fa-solid fa-triangle-exclamation text-base"></i>
                    <div>
                        <div>SYSTEM MAINTENANCE ACTIVE</div>
                        <div class="font-normal text-[11px] text-amber-600/90 dark:text-amber-400/90">Student and Teacher portals are offline. Administrator access only.</div>
                    </div>
                </div>
            <?php endif; ?>
        ...
        <div id="login-box" class="w-full max-w-md space-y-5 form-fade <?php echo ($active_form === 'login') ? '' : 'hidden'; ?>">
            ...
            <form action="index.php" method="POST" onsubmit="showSkeleton()" class="space-y-4">
                ...
                <div class="space-y-1">
                    <label class="text-[10px] font-bold uppercase tracking-wider text-stone-600 dark:text-stone-400">Email Address or Username</label>
        ...
        <?php if (!$maintenanceMode): ?>
            <div id="register-box" class="w-full max-w-md space-y-4 form-fade overflow-y-auto max-h-[calc(100vh-100px)] custom-scrollbar pr-1 <?php echo ($active_form === 'register') ? '' : 'hidden'; ?>">
                <div>
                    <span class="text-[10px] font-black tracking-widest text-orange-500 uppercase">Student Registration</span>
                    <h2 class="serif-title text-2xl font-bold text-stone-900 dark:text-stone-100 mt-0.5">Register Student Account</h2>
                </div>
                ...
                <form action="index.php" method="POST" onsubmit="showSkeleton()" class="space-y-3">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <div class="space-y-1">
                            ...
                            <div class="flex items-center bg-white dark:bg-stone-900 border border-stone-200 dark:border-stone-800 rounded-xl px-3 py-2">
                                <input type="text" id="reg_fullname" name="fullname" required placeholder="Juan Dela Cruz" class="w-full bg-transparent outline-none text-stone-800 dark:text-stone-100 text-xs">
                            </div>
                        </div>
                        <div class="space-y-1">
                            <label class="text-[10px] font-bold uppercase tracking-wider text-stone-600 dark:text-stone-400">Username</label>
                            <div class="flex items-center bg-white dark:bg-stone-900 border border-stone-200 dark:border-stone-800 rounded-xl px-3 py-2">
                                <input type="text" id="reg_username" name="username" required placeholder="juan2026" class="w-full bg-transparent outline-none text-stone-800 dark:text-stone-100 text-xs">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="space-y-1">
                        ...
                        <div class="flex items-center bg-white dark:bg-stone-900 border border-stone-200 dark:border-stone-800 rounded-xl px-3 py-2">
                            <input type="email" id="reg_email" name="email" required placeholder="juan@questbank.edu.ph" class="w-full bg-transparent outline-none text-stone-800 dark:text-stone-100 text-xs">
                        </div>
                    </div>
                    ...
                    <div id="student-fields" class="space-y-3 border-l-2 border-orange-500 pl-3 py-1">
                        <div class="grid grid-cols-2 gap-3">
                            <div>
                                ...
                                <input type="text" id="student_number" name="student_number" required placeholder="2026-00123" class="w-full bg-white dark:bg-stone-900 border rounded-lg p-2 text-xs">
                            </div>
                            <div>
                                <label class="text-[10px] font-bold uppercase text-stone-500">Course</label>
                                <input type="text" id="course" name="course" required placeholder="BSCE" class="w-full bg-white dark:bg-stone-900 border rounded-lg p-2 text-xs">
                            </div>
                        </div>
                        <div class="grid grid-cols-2 gap-3">
                            <div>
                                <label class="text-[10px] font-bold uppercase text-stone-500">Year Level</label>
                                <select id="year_level" name="year_level" required class="w-full bg-white dark:bg-stone-900 border rounded-lg p-2 text-xs">
                            ...
                            <div>
                                ...
                                <input type="text" id="section" name="section" required placeholder="A" class="w-full bg-white dark:bg-stone-900 border rounded-lg p-2 text-xs">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <div class="space-y-1">
                            <label class="text-[10px] font-bold uppercase tracking-wider text-stone-600 dark:text-stone-400">Password</label>
                            <input type="password" id="reg_password" name="password" required placeholder="••••••••" class="w-full bg-white dark:bg-stone-900 border border-stone-200 dark:border-stone-800 rounded-xl p-2 text-xs">
                        </div>
                        <div class="space-y-1">
                            <label class="text-[10px] font-bold uppercase tracking-wider text-stone-600 dark:text-stone-400">Confirm Password</label>
                            <input type="password" id="confirm_password" name="confirm_password" required placeholder="••••••••" class="w-full bg-white dark:bg-stone-900 border border-stone-200 dark:border-stone-800 rounded-xl p-2 text-xs">
                        </div>
                    </div>
